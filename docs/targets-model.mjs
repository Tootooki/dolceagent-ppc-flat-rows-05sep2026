  ...
  const groups=['BROAD','EXACT','ASIN','PHRASE'].map((match,g)=>{
    ...
    const children=Array.from({length:2},(_,k)=>{
      ...
      const context={campaign,group:groupName,sku:product.sku,item:product.item,adType:'SP',
        portfolio:product.name,match:isAsin?'PRODUCT':match,status,target:name,targetKind:isAsin?'ASINs':'Keywords',bid,budget:product.budget,targetAcos:.3};
...
export const createTargetsState=()=>({kind:'All',period:'30D',query:'',targetWidthRatio:null,date:TARGET_DATE_END,rangeStart:'2026-08-23',rangeEnd:TARGET_DATE_END});
...
export function filteredTargets(state,status='All'){
  ...
  const filter=node=>{
    if(!node.children.length){
      ...
      const text=[node.name,node.target,node.campaign,node.portfolio,node.group,node.sku,node.item,node.match].join(' ').toLowerCase();
...
function flatRecords(tree,view){
...
export function targetRows(state,status='All',view='Targets'){
  ...
  return flatRecords(filteredTargets(state,status),view).map(node=>({...node,performance:periods.map(period=>targetPerformance(node,period))}));
}
export function targetsView(state,status='All',view='Targets'){
  ...
  const headers=[identity,'Type',...periods.flatMap(period=>period.metrics.map(metric=>`${period.key}_${metric.label}`)), 'Record type','Match','Status','Bid','ASIN / SKU','Campaign','Ad group'];
  return {id:'ppc',kind:'targets',title:view.toUpperCase(),headers,periods,targetRecords,totalRecord,
    rows:targetRecords.map(row=>[targetLabel(row),TARGET_RECORD_TYPES[row.type],
      ...periods.flatMap((period,i)=>period.metrics.map(metric=>formatTargetMetric(metric,row.performance[i][metric.key]))),
      TARGET_RECORD_TYPES[row.type],row.match||'—',row.status,row.bid===undefined?'—':formatTargetMetric({format:'money'},row.bid),row.sku||'—',row.campaign||'—',row.group||'—']),
