import {TABS, CONFIG, createWorkspaceState, selectWorkspace, setOption, viewFor} from './workspace-model.mjs?v=81';
...
import {createTargetsUI} from './targets-ui.mjs?v=81';
...
const targetUI=createTargetsUI({state:state.targets,getStatus:()=>state.options.ppc.status,getView:()=>state.options.ppc.view,
  onKind:kind=>{if(['Targets','ASINs'].includes(state.options.ppc.view))state.options.ppc.view=kind==='ASINs'?'ASINs':'Targets';syncPpcControls();},
});
// Pages stay directly under the header, independently of the PPC settings panel.
...
targetRail.append(targetPrevious,targetTabs,targetNext);shell.querySelector('.brand-header').after(targetRail);document.body.classList.add('has-page-slider');
...
function openTargetWorkspace(id){
  ...
  targetTabButtons.find(el=>el.dataset.targetWorkspace===id).focus({preventScroll:true});
}
...
const ppcGeneral=node('div','ppc-general-controls'),ppcContent=node('div','ppc-content');
const ppcButtons=[];let ppcControlsOpen=false,ppcPageVisited=false;
for(const field of CONFIG.ppc.fields){
  const group=node('fieldset','workspace-choice-group');group.append(node('legend','',field.label));
  const list=node('div','workspace-choice-list');
  for(const value of field.choices){
    const el=button(value.toUpperCase(),()=>{setOption(state,'ppc',field.key,value);paintWorkspace();},'workspace-choice');
    el.dataset.ppcSetting=field.key;el.dataset.ppcValue=value;ppcButtons.push(el);list.append(el);
  }
  group.append(list);ppcGeneral.append(group);
}
ppcPanel.append(ppcGeneral,targetUI.controls);
const burger=document.getElementById('header-menu');
function syncPpcControls(){
  ppcButtons.forEach(el=>el.setAttribute('aria-pressed',String(state.options.ppc[el.dataset.ppcSetting]===el.dataset.ppcValue)));
